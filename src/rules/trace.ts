...
export const buildRulesTrace = ({ weapon, target, damageReduction = 0 }: CombatRequest): ReadonlyArray<string> => {
  ...
  return [
    ...
    `AP ${weapon.armourPenetration}: effective save ${effectiveSaveTarget(target, weapon.armourPenetration)}+${target.saveModifier === undefined || target.saveModifier === 0 ? '' : ` with ${target.saveModifier >= 0 ? '+' : ''}${target.saveModifier} roll modifier`}`,
    target.rerollSaves === undefined || target.rerollSaves === 'none' ? null : `Save re-rolls: ${target.rerollSaves}`,
