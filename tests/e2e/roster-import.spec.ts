...
test('a pasted roster can be checked, saved and used in a game', async ({ page }) => {
  ...
  await expect(page.getByText('8 wounds remaining', { exact: true })).toBeVisible()
})
...
test('incomplete input remains available for correction', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Add attacking weapon' })).toBeVisible()
})
